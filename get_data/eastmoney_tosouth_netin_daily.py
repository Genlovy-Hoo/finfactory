...
COLS_FINAL = ['date', '港股通(沪)', '港股通(深)', '南下资金']
...
def check_loss(data, trade_dates, logger=None):
    ...
    if len(loss_dates) > 0:
        # logger_show('南下资金净流入日数据有缺失日期：'+','.join(loss_dates),
        #             logger, 'warn')
        logger_show('南下资金净流入日数据缺失数：{}'.format(len(loss_dates)),
                    logger, 'warn')
    return loss_dates


def get_tosouth_netin():
    '''
    | 爬取东财南下资金净流入数据
    | https://data.eastmoney.com/hsgtcg/lz.html
    '''
    url = ('https://push2his.eastmoney.com/api/qt/kamt.kline/get?'
           'fields1=f2,f4,f6&fields2=f51,f52&klt=101&lmt=50000')
    ...
    data = eval(data)['data']
    df_sh2hk = pd.DataFrame([x.split(',') for x in data['sh2hk']],
                            columns=['date', '港股通(沪)']).set_index('date')
    df_sz2hk = pd.DataFrame([x.split(',') for x in data['sz2hk']],
                            columns=['date', '港股通(深)']).set_index('date')
    df_n2s = pd.DataFrame([x.split(',') for x in data['n2s']],
                          columns=['date', '南下资金']).set_index('date')
    data = pd.concat((df_sh2hk, df_sz2hk, df_n2s), axis=1)
    data = data.astype(float)
    data.reset_index(inplace=True)
    # for col in ['港股通(沪)', '港股通(深)', '南下资金']:
    #     data[col] = data[col].astype(float) / 10000
    data = data[COLS_FINAL].copy()
    return data


def update_tosouth_netin_daily(df_exist=None, fpath=None, logger=None):
    '''
    | 从东方财富更新南下资金净流入日数据
...
def update_tosouth_netin_daily_check(save_path=None,
                                     root_dir=None,
                                     trade_dates=None,
                                     logger=None):
    '''从东方财富更新南下资金净流入日数据'''
    ...
    def _get_save_path(save_path):
        ...
        if isnull(save_path):
            ...
            save_dir = find_target_dir('tosouth_money/eastmoney/',
                       root_dir=root_dir, make=True, logger=logger)
            save_path = save_dir + 'tosouth_money_netin.csv'
        return save_path
    
    save_path = _get_save_path(save_path)
    df_all = update_tosouth_netin_daily(df_exist=None,
                                  fpath=save_path,
                                  logger=logger)
    ...
    if is_new:
        logger_show('南下资金净流入日数据更新完成！', logger, 'info')
    else:
        logger_show('南下资金净流入日数据更新未完成！数据最后日期：{}'.format(info[1]),
                    logger, 'warn')
...
if __name__ == '__main__':
    ...
    def try_update_tosouth_netin_daily_check(*args, **kwargs):
        return update_tosouth_netin_daily_check(*args, **kwargs)
    
    trade_dates = load_trade_dates_tushare('SSE')
    # trade_dates = None
    
    df, loss = try_update_tosouth_netin_daily_check(
                                        save_path=None,
                                        root_dir=None,
