...
COLS_FINAL = ['code', 'date', 'open', 'high', 'low',
              'close', 'pre_close', 'change', 'change_pct',
              'volume', 'amount']
...
def get_fund_daily(code, start_date, end_date=None, ts_api=None):
    '''
    | tushare获取基金日线数据
    ...
    data = []
    for date1, date2 in dttools.cut_date(start_date, end_date, 700):
    ...
    df['amount'] = df['amount'] * 1000 # tushare基金行情成交额单位为千元
...
def update_fund_daily(code, df_exist=None, fpath=None,
    ...
    logger_show('更新{}日线数据, {}->{} ...'.format(code, dates[0], dates[-1]),
                logger, 'info')
    data = get_fund_daily(code, start_date, end_date, ts_api)
    if data.shape[0] == 0:
...
def update_fund_daily_check(code,
                            save_path=None,
                            root_dir=None,
                            start_date='19901218',
                            trade_dates=None,
    ...
    data_all = update_fund_daily(code,
                                 df_exist=None,
                                 fpath=save_path,
...
if __name__ == '__main__':
    ...
    def try_update_fund_daily_check(*args, **kwargs):
        return update_fund_daily_check(*args, **kwargs)
    ...
    for k in range(len(keys)):
        ...
        exec('''dfs['{}'], losses['{}'] = try_update_fund_daily_check(
                                    code,
                                    save_path=None,
                                    root_dir=None,
                                    start_date=start_date,
