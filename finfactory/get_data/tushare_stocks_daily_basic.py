...
def get_stocks_daily_basic(start_date=None, end_date=None,
...
def update_stocks_daily_basic(df_exist=None,
                              start_date=None,
                              end_date=None,
                              fpath=None,
                              trade_dates=None,
    ...
    if last_date >= end_date:
        logger_show('股票日线基本数据最新数据已存在，不更新。', logger)
        return df_exist
    ...
    logger_show('更新股票日线基本数据, {}->{} ...'.format(start_date, end_date),
                logger, 'info')
    data = get_stocks_daily_basic(start_date=start_date,
                                  end_date=end_date,
                                  fpath=None,
...
def update_stocks_daily_basic_check(save_path=None,
                                    start_date=None,
                                    root_dir=None,
                                    trade_dates=None,
                                    split=True,
    ...
    def _get_save_path(save_path):
        ...
        if isnull(save_path):
            ...
            save_path = '{}astocks_daily_basic.csv'.format(save_dir)
        return save_path
    ...
    data_all = update_stocks_daily_basic(df_exist=None,
    ...
    # 数据拆分为为每只股票一个文件
    codes = data_all['code'].unique().tolist()
...
if __name__ == '__main__':
    ...
    def try_update_stocks_daily_basic_check(*args, **kwargs):
        return update_stocks_daily_basic_check(*args, **kwargs)
    
    
    fdir = find_target_dir('stocks/tushare/stocks_daily/',
                           root_dir=None, logger=logger)
    fpath = '{}astocks_daily_basic.csv'.format(fdir)
    ...
    data_all, loss_dates, df_stocks, loss_stocks = \
              try_update_stocks_daily_basic_check(
                                save_path=None,
                                start_date='20220101',
                                root_dir=None,
                                trade_dates=trade_dates,
