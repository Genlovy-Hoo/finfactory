...
def get_cols_info(fpath=None, root_dir=None):
    ...
    if isnull(fpath):
        fdir = find_target_dir('stocks/tushare/',
                               root_dir=root_dir)
        fpath = '{}cols_zjc.xlsx'.format(fdir)
...
def get_fpath(code, fpath, save_csv, root_dir):
    ...
    return '{}{}_zjc.csv'.format(fdir, code)
...
def get_stock_zjc(code, start_date=None, end_date=None,
                  fpath=None, save_csv=False, root_dir=None,
    ...
    for col in ['change_vol', 'change_ratio', 'after_share',
                'after_ratio', 'avg_price', 'total_share']:
        df[col] = df[col].apply(lambda x: round(x, 4) if not isnull(x) else np.nan)
    df.rename(columns={'ts_code': 'code', 'ann_date': '公告日期'},
              inplace=True)
    if not isnull(COLS):
        df.rename(columns=COLS_MAP, inplace=True)
        df = df[COLS]
    df.sort_values(['code', '公告日期', 'holder_name'], inplace=True)
    df.reset_index(drop=True, inplace=True)
    _write_to_csv(df, code, fpath, save_csv, root_dir)
    return df


def update_stock_zjc(code, df_exist=None,
    ...
    if last_date >= end_date:
        logger_show('{}股东增减持最新数据已存在，不更新。'.format(code),
                    logger, 'info')
        return df_exist
    
    logger_show('更新{}股东增减持数据, {}->{} ...'.format(code, start_date, end_date),
                logger, 'info')
    data = get_stock_zjc(code,
    ...
    data_all = archive_data(data, df_exist,
                            sort_cols=['code', '公告日期',
                                       'holder_name'],
                            del_dup_cols=list(data.columns),
                            sort_first=False,
                            csv_path=fpath,
                            csv_index=None,
                            csv_encoding='gbk')
...
def get_save_path(code, save_path, root_dir, logger=None):
    ...
    if isnull(save_path):
        ...
        save_path = '{}{}_zjc.csv'.format(save_dir, code)
    return save_path


def update_stock_zjc_check(code,
                           save_path=None,
                           start_date=None,
                           root_dir=None,
                           ts_api=None,
    ...
    data_all = update_stock_zjc(code,
                                df_exist=None,
                                start_date=start_date,
                                end_date=None,
                                fpath=save_path,
...
if __name__ == '__main__':
    ...
    def try_update_stock_zjc_check(*args, **kwargs):
        return update_stock_zjc_check(*args, **kwargs)
    ...
    for k in range(len(keys)):
        ...
        logger_show('{}/{}, {}...'.format(k+1, len(keys), code),
                    logger)
        exec('''dfs['{}'] = try_update_stock_zjc_check(
                                    code,
                                    save_path=None,
                                    start_date=start_date,
                                    root_dir=None,
