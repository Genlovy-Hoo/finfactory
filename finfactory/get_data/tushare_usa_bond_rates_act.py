...
COLS_FINAL = ['date', 'y5', 'y7', 'y10', 'y20', 'y30']
...
def check_loss(data, logger=None):
    ...
    loss_dates = check_date_loss(data, only_workday=False,
                                 del_weekend=True)
    if len(loss_dates) > 0:
        # logger_show('美国债实际收益率日数据有缺失日期：'+','.join(loss_dates),
        #             logger, 'warn')
        logger_show('美国债实际收益率日数据缺失数：'+str(len(loss_dates)),
                    logger, 'warn')
    return loss_dates


def get_usa_bonds_act(start_date, end_date=None, ts_api=None):
    '''
...
def update_usa_bonds_act(df_exist=None, fpath=None,
                         start_date='20030101', end_date=None,
    ...
    if last_date >= end_date:
        logger_show('美国债实际收益率最新数据已存在，不更新。', logger)
        return df_exist
    
    logger_show('更新美国债实际收益率数据, {}->{} ...'.format(start_date, end_date),
                logger, 'info')
    data = get_usa_bonds_act(start_date, end_date, ts_api)
    if data.shape[0] == 0:
...
def update_usa_bonds_act_check(save_path=None,
                               root_dir=None,
                               start_date='20030101',
                               ts_api=None,
                               logger=None):
    ...
    def _get_save_path(save_path):
        ...
        if isnull(save_path):
            ...
            save_path = save_dir + 'usa_bond_rates_act.csv'
        return save_path
    ...
    data_all = update_usa_bonds_act(df_exist=None,
                                    fpath=save_path,
    ...
    logger_show('美国债实际收益率数据最新日期: {}。'.format(data_all['date'].max()), logger)
...
if __name__ == '__main__':
    ...
    def try_update_usa_bonds_act_check(*args, **kwargs):
        return update_usa_bonds_act_check(*args, **kwargs)
    
    
    
    df, loss = try_update_usa_bonds_act_check(
                                save_path=None,
                                root_dir=None,
                                start_date='20030101',
                                ts_api=ts_api,
