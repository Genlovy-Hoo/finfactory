...
def get_cols_info(fpath=None, root_dir=None):
    ...
    if isnull(fpath):
        fdir = find_target_dir('stocks/tushare/',
                               root_dir=root_dir)
        fpath = '{}cols_top10holders_free.xlsx'.format(fdir)
...
def get_fpath(code, fpath, save_csv, root_dir):
    ...
    return '{}{}_top10holders_free.csv'.format(fdir, code)
...
def get_stock_top10holders_free(code,
                                start_date=None,
                                end_date=None,
    ...
    for dt1, dt2 in dttools.cut_date(start_date, end_date, 1000):
        global TS_API_USED_TIMES
        TS_API_USED_TIMES += 1
        if TS_API_USED_TIMES % cfg.ts_1min_top10holders_free == 0:
            logger_show('{}, {}, pausing...'.format(code, dt1),
                        logger)
            time.sleep(61)
        df = ts_api.top10_floatholders(ts_code=code,
                                       start_date=dt1,
                                       end_date=dt2,
                                       fields=FIELDS)
        data.append(df)
...
def update_stock_top10holders_free(code,
                                   df_exist=None,
                                   start_date=None,
                                   end_date=None,
                                   fpath=None,
    ...
    if last_date >= end_date:
        logger_show('{}前十大流通股东最新数据已存在，不更新。'.format(code),
                    logger, 'info')
        return df_exist
    
    logger_show('更新{}前十大流通股东数据, {}->{} ...'.format(code, start_date, end_date),
                logger, 'info')
    data = get_stock_top10holders_free(code,
...
def get_save_path(code, save_path, root_dir, logger=None):
    ...
    if isnull(save_path):
        ...
        save_path = '{}{}_top10holders_free.csv'.format(save_dir, code)
    return save_path


def update_stock_top10holders_free_check(code,
                                         save_path=None,
                                         start_date=None,
                                         root_dir=None,
                                         ts_api=None,
    ...
    data_all = update_stock_top10holders_free(
                                        code,
                                        df_exist=None,
                                        start_date=start_date,
                                        end_date=None,
...
if __name__ == '__main__':
    ...
    def try_update_stock_top10holders_free_check(*args, **kwargs):
        return update_stock_top10holders_free_check(*args, **kwargs)
    ...
    for k in range(len(keys)):
        ...
        logger_show('{}/{}, {}...'.format(k+1, len(keys), code),
                    logger)
        exec('''dfs['{}'] = try_update_stock_top10holders_free_check(
                                    code,
                                    save_path=None,
                                    start_date=start_date,
                                    root_dir=None,
