...
def get_cols_info(fpath=None, root_dir=None):
    '''读取字段信息'''
    if isnull(fpath):
        fdir = find_target_dir('fund/tushare/',
                               root_dir=root_dir)
        fpath = '{}cols_fund_list.xlsx'.format(fdir)
...
def get_fund_list(cols_path=None, root_dir=None, ts_api=None):
    '''
    tushare获取基金列表数据
    '''
    if isnull(ts_api):
    ...
    part1 = ts_api.fund_basic(market='O', fields=fields)
    part2 = ts_api.fund_basic(market='E', fields=fields)
    df = pd.concat((part1, part2), axis=0)
    if not isnull(cols):
        df.rename(columns=cols_map, inplace=True)
        df = df[cols]
    for col in ['found_date', 'due_date', 'list_date',
...
def update_fund_list(save_path=None, root_dir=None,
    ...
    if save_path is None:
        save_dir = find_target_dir('fund/tushare/',
                   root_dir=root_dir, make=True, logger=logger)
        save_path = save_dir + 'fund_list.csv'
    ...
    if os.path.exists(save_path) and \
       (get_last_change_time(save_path, '%Y-%m-%d') >= end_date):
        logger_show('基金列表数据已是最新，不更新！', logger)
        return load_csv(save_path, encoding='gbk')
    
    logger_show('更新基金列表数据...', logger)
    df = get_fund_list(cols_path, root_dir, ts_api)
    ...
    df = archive_data(df, df_exist,
                      sort_cols=['code', 'name'],
                      del_dup_cols=['code', 'name', 'found_date'],
                      sort_first=False,
                      csv_path=save_path,
                      csv_index=None,
                      csv_encoding='gbk')
...
if __name__ == '__main__':
    ...
    def try_update_fund_list(*args, **kwargs):
        return update_fund_list(*args, **kwargs)
    
    
    df = try_update_fund_list(
                            save_path=None,
                            root_dir=None,
                            cols_path=None,
                            ts_api=ts_api,
