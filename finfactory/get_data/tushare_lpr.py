...
def check_loss(data, logger=None):
    ...
    loss_dates = check_date_loss(data, only_workday=False,
                                 del_weekend=True)
    if len(loss_dates) > 0:
        # logger_show('LPR数据有缺失日期：'+','.join(loss_dates),
        #             logger, 'warn')
        logger_show('LPR数据缺失数：'+str(len(loss_dates)),
                    logger, 'warn')
    return loss_dates


def get_lpr(start_date, end_date=None, ts_api=None):
    '''
    tushare获取LPR数据
    '''
    if isnull(ts_api):
...
def update_lpr(df_exist=None, fpath=None,
               start_date='20131024', end_date=None,
    ...
    if last_date >= end_date:
        logger_show('LPR最新数据已存在，不更新。', logger)
        return df_exist
    
    logger_show('更新LPR数据, {}->{} ...'.format(start_date, end_date),
                logger, 'info')
    data = get_lpr(start_date, end_date, ts_api)
    if data.shape[0] == 0:
...
def update_lpr_check(save_path=None,
                     root_dir=None,
                     start_date='20131024',
                     ts_api=None,
                     logger=None):
    ...
    def _get_save_path(save_path):
        ...
        if isnull(save_path):
            ...
            save_dir = find_target_dir('lpr/tushare/',
                       root_dir=root_dir, make=True, logger=logger)
            save_path = save_dir + 'lpr.csv'
        return save_path
    ...
    data_all = update_lpr(df_exist=None,
                          fpath=save_path,
    ...
    logger_show('LPR利率数据最新日期: {}。'.format(data_all['date'].max()), logger)
...
if __name__ == '__main__':
    ...
    def try_update_lpr_check(*args, **kwargs):
        return update_lpr_check(*args, **kwargs)
    
    
    
    df, loss = try_update_lpr_check(
                                save_path=None,
                                root_dir=None,
                                start_date='20131024',
                                ts_api=ts_api,
