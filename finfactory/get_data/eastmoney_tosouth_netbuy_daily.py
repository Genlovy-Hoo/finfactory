...
def check_loss(data, trade_dates, logger=None):
    ...
    if len(loss_dates) > 0:
        # logger_show('南下资金净买入日数据有缺失日期：'+','.join(loss_dates),
        #             logger, 'warn')
        logger_show('南下资金净买入日数据缺失数：{}'.format(len(loss_dates)),
                    logger, 'warn')
    return loss_dates


def get_tosouth_netbuy():
    '''
    | 爬取东财南下资金净买入数据
    ...
    url = 'https://datacenter-web.eastmoney.com/securities/api/data/get?' + \
          ...
          'NET_INFLOW_BOTH,TIME_TYPE&filter=(DIRECTION_TYPE="2")' + \
...
def update_tosouth_netbuy_daily(df_exist=None, fpath=None, logger=None):
    '''
    | 从东方财富更新南下资金净买入日数据
...
def update_tosouth_netbuy_daily_check(save_path=None,
                                      root_dir=None,
                                      trade_dates=None,
                                      logger=None):
    '''从东方财富更新南下资金净买入日数据'''
    ...
    def _get_save_path(save_path):
        ...
        if isnull(save_path):
            ...
            save_path = save_dir + 'tosouth_money_netbuy.csv'
        return save_path
    
    save_path = _get_save_path(save_path)
    df_all = update_tosouth_netbuy_daily(df_exist=None,
                                  fpath=save_path,
                                  logger=logger)
    ...
    if is_new:
        logger_show('南下资金净买入日数据更新完成！', logger, 'info')
    else:
        logger_show('南下资金净买入日数据更新未完成！数据最后日期：{}'.format(info[1]),
                    logger, 'warn')
...
if __name__ == '__main__':
    ...
    def try_update_tosouth_netbuy_daily_check(*args, **kwargs):
        return update_tosouth_netbuy_daily_check(*args, **kwargs)
    
    trade_dates = load_trade_dates_tushare('SSE')
    # trade_dates = None
    
    df, loss = try_update_tosouth_netbuy_daily_check(
                                        save_path=None,
                                        root_dir=None,
