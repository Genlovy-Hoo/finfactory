...
# LOGGER = None
LOGGER = simple_logger('../../log/get_data_ccxt_1min_ba.log', 'a')
...
def update_minute(symbol, fpath, start_time=None, end_time=None):
    assert start_time is not None or os.path.exists(fpath), '开始时间或本地文件必须存在一个'
    if os.path.exists(fpath):
        df = load_csv(fpath)
    else:
        df = get_minute(symbol, start_time=start_time)
    ...
    while not update_now:
        df_ = get_minute(symbol, start_time=time_last)
...
def check_minute(df):
    ...
    times_all = pd.date_range(time_min, time_max, freq='1min')
...
if __name__ == '__main__':
    ...
    fpath = '../../data/archives/eth/binance/eth_1minute.csv'
    logger_show('update {} minute ...'.format(symbol), LOGGER, 'info')
    df_eth = update_minute(symbol, fpath, start_time)
    ...
    fpath = '../../data/archives/btc/binance/btc_1minute.csv'
    logger_show('update {} minute ...'.format(symbol), LOGGER, 'info')
    df_btc = update_minute(symbol, fpath, start_time)
