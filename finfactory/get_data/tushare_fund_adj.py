...
COLS_FINAL = ['code', 'date', 'adj_factor']
...
def check_loss(data, code, trade_dates, logger=None):
    ...
    if len(loss_dates) > 0:
        logger_show('{}复权因子数据有缺失日期：'.format(code)+','.join(loss_dates),
                    logger, 'warn')
    return loss_dates


def get_fund_adj(code, start_date, end_date=None, ts_api=None):
    '''
    | tushare获取基金复权因子数据
    ...
    data = []
    for date1, date2 in dttools.cut_date(start_date, end_date, 1000):
        df = ts_api.fund_adj(ts_code=code,
                             start_date=date1,
                             end_date=date2,
...
def update_fund_adj(code, df_exist=None, fpath=None,
    ...
    if last_date >= end_date:
        logger_show('{}复权因子最新数据已存在，不更新。'.format(code),
                    logger, 'info')
        return df_exist
    ...
    logger_show('更新{}复权因子数据, {}->{} ...'.format(code, dates[0], dates[-1]),
                logger, 'info')
    data = get_fund_adj(code, start_date, end_date, ts_api)
    if data.shape[0] == 0:
...
def update_fund_adj_check(code,
                          save_path=None,
                          root_dir=None,
                          start_date='19901218',
                          trade_dates=None,
    ...
    def _get_save_path(save_path):
        ...
        if isnull(save_path):
            ...
            save_dir = find_target_dir('fund/tushare/{}/'.format(code),
                       root_dir=root_dir, make=True, logger=logger)
            save_path = save_dir + '{}_adj.csv'.format(code)
        return save_path
...
if __name__ == '__main__':
    ...
    def try_update_fund_adj_check(*args, **kwargs):
        return update_fund_adj_check(*args, **kwargs)
    
    
    # tushare代码
    codes = {
        '510050.SH': '2005-02-22', # '上证50ETF'
        '510300.SH': '2012-05-27', # '沪深300（沪）ETF'
        '159919.SZ': '2012-05-27', # '沪深300（深）ETF'
        '510500.SH': '2013-03-14', # '中证500ETF'
    }
    ...
    for k in range(len(keys)):
        ...
        exec('''dfs['{}'], losses['{}'] = try_update_fund_adj_check(
                                    code,
                                    save_path=None,
                                    root_dir=None,
                                    start_date=start_date,
