...
def get_stock_daily_stk(code, start_date=None, end_date=None,
    ...
    for dt1, dt2 in dttools.cut_date(start_date, end_date, 4500):
        df = ts_api.stk_factor(ts_code=code,
                               start_date=dt1,
                               end_date=dt2,
                               fields=FIELDS)
        data.append(df)
...
def update_stock_daily_stk(code, df_exist=None,
    ...
    logger_show('更新{}日线数据, {}->{} ...'.format(code, start_date, end_date),
                logger, 'info')
    data = get_stock_daily_stk(
...
def update_stock_daily_stk_check(code,
                                 save_path=None,
                                 start_date=None,
                                 root_dir=None,
                                 trade_dates=None,
    ...
    def _get_save_path(save_path):
        ...
        if isnull(save_path):
            ...
            save_path = '{}{}_daily_stk.csv'.format(save_dir, code)
        return save_path
    ...
    data_all = update_stock_daily_stk(code,
                                      df_exist=None,
...
if __name__ == '__main__':
    ...
    def try_update_stock_daily_stk_check(*args, **kwargs):
        return update_stock_daily_stk_check(*args, **kwargs)
    ...
    for k in range(len(keys)):
        ...
        logger_show('{}/{}, {}...'.format(k+1, len(keys), code),
                    logger)
        exec('''dfs['{}'], losses['{}'] = try_update_stock_daily_stk_check(
                                            code,
                                            save_path=None,
                                            start_date=start_date,
                                            root_dir=None,
