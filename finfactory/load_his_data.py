...
def load_trade_dates_tushare(exchange='SSE', root_dir=None):
    '''读取交易所交易日历历史数据'''
    fdir = find_target_dir('trade_dates/', root_dir=root_dir)
...
#%%
def load_chn_national_bond_yields(root_dir=None):
    '''读取国债收益率历史数据'''
    fdir = find_target_dir('chn_national_bond_rates/',
                           root_dir=root_dir)
    fpath = fdir+'chn_national_bond_rates.csv'
    df = load_csv(fpath, encoding='gbk')
...
def load_cffex_lhb_future(code, date, root_dir=None):
    '''读取中金所期货龙虎榜数据'''
    fdir = find_target_dir('futures/lhb/cffex/{}/'.format(code),
                           root_dir=root_dir)
    date = date_reformat(date, '')
    save_path = '{}{}{}.csv'.format(fdir, code, date)
    df = load_text_multi(save_path, encoding='gbk')
    return df
